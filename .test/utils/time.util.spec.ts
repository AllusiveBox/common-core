...
/**
 *
 * Test Suite for all the functions included in the Time Util sub package.
 *
 * @group unit
 * @group utils
 * @group time
...
describe("TimeUtil Unit Test Suite", () => {
	...
	("that given %s, attempting to calculate milliseconds throws an error", (arg) => {
		expect(() => {
			// @ts-ignore
			TimeUtil.Milliseconds.inSeconds(arg)
		}).toThrowError(`Cannot convert type: ${typeof arg} to milliseconds; Must be of type number`);
	});

	test.each(invalidList)
	("that given %s, attempting to convert from milliseconds throws an error", (arg) => {
		expect(() => {
			// @ts-ignore
			TimeUtil.Milliseconds.toSeconds(arg)
		}).toThrowError(`Cannot convert type: ${typeof arg} from milliseconds; Must be of type number`);
	});
	...
	("that given $hours hours, inHours returns $milliseconds milliseconds", ({ hours, milliseconds }) => {
		expect(TimeUtil.Milliseconds.inHours(hours)).toBe(milliseconds);
	});

	test("that not given any hours, inHours returns 3.6e6 milliseconds", () => {
		expect(TimeUtil.Milliseconds.inHours()).toBe(3.6e6);
	});

	test.each(hoursList)
	("that given $milliseconds, toHours returns $hours hours", ({ milliseconds, hours }) => {
		expect(TimeUtil.Milliseconds.toHours(milliseconds)).toBe(hours);
	});

	test("that not given any milliseconds, toHours returns 0 milliseconds", () => {
		expect(TimeUtil.Milliseconds.toHours()).toBe(0);
	});
	...
	("that given $days days, inDays returns $milliseconds milliseconds", ({ days, milliseconds }) => {
		expect(TimeUtil.Milliseconds.inDays(days)).toBe(milliseconds);
	});
	...
	("that given $milliseconds, toDays returns $days days", ({ milliseconds, days }) => {
		expect(TimeUtil.Milliseconds.toDays(milliseconds)).toBe(days);
	});
	...
	("that given $weeks weeks, inWeeks returns $milliseconds milliseconds", ({ weeks, milliseconds }) => {
		expect(TimeUtil.Milliseconds.inWeeks(weeks)).toBe(milliseconds);
	});
	...
	("that given $milliseconds milliseconds, toWeeks returns $weeks weeks", ({ milliseconds, weeks }) => {
		expect(TimeUtil.Milliseconds.toWeeks(milliseconds)).toBe(weeks);
	});

	test("that given no milliseconds, toWeeks returns 0 weeks", () => {
		expect(TimeUtil.Milliseconds.toWeeks()).toBe(0);
	});
	...
	("that given $months months, inMonths returns $milliseconds milliseconds", ({ months, milliseconds }) => {
		expect(TimeUtil.Milliseconds.inMonths(months)).toBe(milliseconds);
	});

	test("that given no months, inMonths returns 2.628e9 milliseconds", () => {
		expect(TimeUtil.Milliseconds.inMonths()).toBe(2.628e9);
	});

	test.each(monthsList)
	("that given $milliseconds milliseconds, toMonths returns $months months", ({ milliseconds, months }) => {
		expect(TimeUtil.Milliseconds.toMonths(milliseconds)).toBe(months);
	});

	test("that given no milliseconds, toMonths returns 0 months", () => {
		expect(TimeUtil.Milliseconds.toMonths()).toBe(0);
	});
	...
	("that given $years years, inYears returns $milliseconds milliseconds", ({ years, milliseconds }) => {
		expect(TimeUtil.Milliseconds.inYears(years)).toBe(milliseconds);
	});

	test("that given no years, inYears returns 3.154e10 milliseconds", () => {
		expect(TimeUtil.Milliseconds.inYears()).toBe(3.154e10);
	});

	test.each(yearsList)
	("that given $milliseconds milliseconds, toYears returns $years years", ({ milliseconds, years }) => {
		expect(TimeUtil.Milliseconds.toYears(milliseconds)).toBe(years);
	});

	test("that given no milliseconds, toYears returns 0 years", () => {
		expect(TimeUtil.Milliseconds.toYears()).toBe(0);
	});
